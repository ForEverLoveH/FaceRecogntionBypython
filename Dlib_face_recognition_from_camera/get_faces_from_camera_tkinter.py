...
class Face_Register:
    def __init__(self):
        ...
        self.font_title = tkFont.Font(family=None, size=20, weight='bold')
        self.font_step_title = tkFont.Font(family=None, size=15, weight='bold')
        ...
        self.cap.set(cv2.CAP_PROP_FPS, 120)  # 帧数
    ...
    def GUI_clear_data(self):
        ...
        if os.path.isfile("data/features_all.csv"):  #存在文件则删除feature 的文件，相当做人脸的更新
            os.remove("data/features_all.csv")
        self.label_cnt_face_in_database['text'] = "0"
        self.existing_faces_cnt = 0
        self.log_all["text"] = "储存的人脸图像 和 `features_all.csv` 已移除!"
    ...
    def GUI_info(self):
        ...
        tk.Label(self.frame_right_info,
                 text="帧数: ").grid(row=1, column=0, columnspan=2, sticky=tk.W, padx=5, pady=2)
        self.label_fps_info.grid(row=1, column=2, sticky=tk.W, padx=5, pady=2)

        tk.Label(self.frame_right_info,
                 text="数据量: ").grid(row=2, column=0, columnspan=2, sticky=tk.W, padx=5, pady=2)
        self.label_cnt_face_in_database.grid(row=2, column=2, columnspan=3, sticky=tk.W, padx=5, pady=2)

        tk.Label(self.frame_right_info,
                 text="人脸数量: ").grid(row=3, column=0, columnspan=2, sticky=tk.W, padx=5, pady=2)
        ...
        tk.Label(self.frame_right_info,
                 ...
                 text="清空人脸数据库(谨慎考虑)").grid(row=5, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)
        ...
        tk.Label(self.frame_right_info,
                 font=self.font_step_title,
                 text="1：输入姓名").grid(row=7, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)
        ...
        tk.Label(self.frame_right_info,
                 font=self.font_step_title,
                 text="2：存储人脸图像").grid(row=9, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)
    ...
    def create_face_folder(self):
        ...
        if self.input_name_char:
            self.current_face_dir = self.path_photos_from_camera + \
                                    ...
                                    self.input_name_char
            # file_handle = open('data/faceName.txt', mode='a', encoding="utf-8")
            # file_handle.write(self.input_name_char + '\n')
        else:
            self.current_face_dir = self.path_photos_from_camera + \
                                    "person_" + str(self.existing_faces_cnt)
            # file_handle = open('data/faceName.txt', mode='a', encoding="utf-8")
            # file_handle.write(str(self.existing_faces_cnt) + '\n')
        os.makedirs(self.current_face_dir)
        self.log_all["text"] = "\"" + self.current_face_dir + "/\" 成功创建文件路径!"
            ...
            if self.current_frame_faces_cnt == 1:
                if not self.out_of_range_flag:
                    ...
                    self.log_all["text"] = "\"" + self.current_face_dir + "/img_face_" + str(
                        self.ss_cnt) + ".jpg\"" + " 人脸图像已存储!"
                    self.face_ROI_image = cv2.cvtColor(self.face_ROI_image, cv2.COLOR_BGR2RGB)

                    cv2.imencode('.jpg', self.face_ROI_image)[1].tofile(
            ...
            if len(faces) != 0:
                ...
                for k, d in enumerate(faces):
                    ...
                    if (d.right() + self.ww) > 640 or (d.bottom() + self.hh > 480) or (d.left() - self.ww < 0) or (
                            d.top() - self.hh < 0):
                        self.label_warning["text"] = "人脸过于靠近!"
                    ...
                    self.current_frame = cv2.rectangle(self.current_frame,
                                                       ...
                                                       color_rectangle, 2)
                    cv2.line(self.current_frame, tuple([d.left() - self.ww, d.top() - self.hh]),
                             tuple([d.left() + 20 - self.ww, d.top() - self.hh]),
                             color_rectangle, 4)
                    cv2.line(self.current_frame, tuple([d.left() - self.ww, d.top() - self.hh]),
                             tuple([d.left() - self.ww, d.top() + 20 - self.hh]),
                             color_rectangle, 4)

                    cv2.line(self.current_frame, tuple([d.right() + self.ww, d.top() - self.hh]),
                             tuple([d.right() - 20 + self.ww, d.top() - self.hh]),
                             color_rectangle, 4)
                    cv2.line(self.current_frame, tuple([d.right() + self.ww, d.top() - self.hh]),
                             tuple([d.right() + self.ww, d.top() + 20 - self.hh]),
                             color_rectangle, 4)

                    cv2.line(self.current_frame, tuple([d.left() - self.ww, d.bottom() + self.hh]),
                             tuple([d.left() + 20 - self.ww, d.bottom() + self.hh]),
                             color_rectangle, 4)
                    cv2.line(self.current_frame, tuple([d.left() - self.ww, d.bottom() + self.hh]),
                             tuple([d.left() - self.ww, d.bottom() - 20 + self.hh]),
                             color_rectangle, 4)

                    cv2.line(self.current_frame, tuple([d.right() + self.ww, d.bottom() + self.hh]),
                             tuple([d.right() - 20 + self.ww, d.bottom() + self.hh]),
                             color_rectangle, 4)
                    cv2.line(self.current_frame, tuple([d.right() + self.ww, d.bottom() + self.hh]),
                             tuple([d.right() + self.ww, d.bottom() - 20 + self.hh]),
                             color_rectangle, 4)
...
def main_end():
    ...
    # 如果第一次存储或者没有之前录入的人脸, 按照 person_1 开始录入 / Start from person_1
        ...
        logging.warning("'data/data_faces_from_camera/' not found!")
