...
c = s.socket(s.AF_INET, s.SOCK_STREAM)  # 定义socket类型，网络通信，TCP

c.connect((HOST, PORT))  # 要连接的IP与端口
...
# faceName_path = "data/faceName.txt"

current_name = "null"  # 识别到的人员名称
...
class Face_Recognizer:
    ...
    #     with open(faceName_path, 'r', encoding='utf-8') as f:
    #         datas = f.readlines()
    #         for line in datas:
    #             self.all_name.append(line[0:-1])
    ...
    #         if len(self.face_name_known_list) == len(self.all_name):
    #             for index in range(len(self.face_name_known_list)):
    #                 self.face_name_known_list[index] = self.all_name[index]
    ...
    def process(self, stream):
        ...
        if self.get_face_database():
            while stream.isOpened():
                    ...
                    if self.current_frame_face_cnt == 1:
                        if self.reclassify_interval_cnt == self.reclassify_interval:
                            ...
                            for k in range(len(faces)):
                                ...
                                # c. 对于某张人脸, 遍历所有存储的人脸特征 / For every face detected, compare it with all the faces in
                                # the database
                                ...
                                if min(self.current_frame_face_X_e_distance_list) < 0.4:
                                    ...
                                    current_name = self.face_name_known_list[similar_person_num]
                                else:
                            ...
                            for k, d in enumerate(faces):
                                cv2.rectangle(img_rd,
                                              ...
                                              (147, 20, 255), 1)
                                cv2.line(img_rd, tuple([d.left(), d.top()]), tuple([d.left() + 20, d.top()]),
                                         (0, 0, 255), 4)
                                cv2.line(img_rd, tuple([d.left(), d.top()]), tuple([d.left(), d.top() + 20]),
                                         (0, 0, 255), 4)

                                cv2.line(img_rd, tuple([d.right(), d.top()]), tuple([d.right() - 20, d.top()]),
                                         (0, 0, 255), 4)
                                cv2.line(img_rd, tuple([d.right(), d.top()]), tuple([d.right(), d.top() + 20]),
                                         (0, 0, 255), 4)

                                cv2.line(img_rd, tuple([d.left(), d.bottom()]), tuple([d.left() + 20, d.bottom()]),
                                         (0, 0, 255), 4)
                                cv2.line(img_rd, tuple([d.left(), d.bottom()]), tuple([d.left(), d.bottom() - 20]),
                                         (0, 0, 255), 4)

                                cv2.line(img_rd, tuple([d.right(), d.bottom()]), tuple([d.right() - 20, d.bottom()]),
                                         (0, 0, 255), 4)
                                cv2.line(img_rd, tuple([d.right(), d.bottom()]), tuple([d.right(), d.bottom() - 20]),
                                         (0, 0, 255), 4)
                ...
                if kk == 27:
                    break
                sendData = '{"cmd": "FaceRecognition", "msg": "' + current_name + '"}\r\n'
                c.send(sendData.encode('utf-8'))
                self.update_fps()

                cv2.namedWindow("Face recognition-Chinese name", 1)
                cv2.imshow("Face recognition-Chinese name", img_rd)
    ...
    def run(self):
        ...
        cap.set(cv2.CAP_PROP_FPS, 120)  # 帧数
        # self.process(cap)
        import threading
        hello_thread = threading.Thread(name="Hello thread", target=self.process(cap))
